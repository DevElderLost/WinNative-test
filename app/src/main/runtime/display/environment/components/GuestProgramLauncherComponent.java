...
public class GuestProgramLauncherComponent extends EnvironmentComponent {
  ...
  private void applyLsfgEnvVars(EnvVars envVars, ImageFs imageFs) {
    ...
    if (dllPath.isEmpty() || !new java.io.File(dllPath).isFile()) {
      envVars.put("DISABLE_LSFG", "1");
      Log.w("GuestProgramLauncherComponent", "LSFG: Lossless.dll not found at: " + dllPath);
      return;
    }
    ...
    if (guestExecutable != null && !guestExecutable.isEmpty()) {
      ...
      for (int i = parts.length - 1; i >= 0; i--) {
        String p = parts[i];
        if (p.toLowerCase(java.util.Locale.ROOT).endsWith(".exe")) {
          processName = new java.io.File(p.replace("\\", "/")).getName();
          break;
        }
      }
    }

    // Buat dirs jika belum ada
    configDir.mkdirs();
    lsfgTmpDir.mkdirs();
