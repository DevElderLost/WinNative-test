...
public class GuestProgramLauncherComponent extends EnvironmentComponent {
  ...
  private int execGuestProgram() {
    ...
    // 1. VK_LAYER_PATH LSFG (per-container) tidak tertimpa oleh VK_LAYER_PATH
    //    global dari XServerDisplayActivity via mergeExternalEnvVars/putAll.
  ...
  private void applyLsfgEnvVars(EnvVars envVars, ImageFs imageFs) {
    ...
    // Tulis conf.toml lengkap ke per-container config dir
    ...
    Log.d("GuestProgramLauncherComponent", "LSFG armed:"
        ...
        + " flowScale=" + flowScale
        + " perfMode=" + perfMode
        + " config=" + confToml.getAbsolutePath());
  }

  private static String clampLsfgInt(String value, int min, int max) {
    try {
      return String.valueOf(Math.max(min, Math.min(max, Integer.parseInt(value))));
